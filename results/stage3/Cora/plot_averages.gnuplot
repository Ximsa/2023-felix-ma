...
do for [FILE in FILES]{
    ...
    i = i + ROWCOUNT
    REPORT = 5
    if (REPORT < ROWCOUNT) {REPORT=ROWCOUNT}
    print sprintf("%s\tMean_Std(after %i|C|): $%.1f_{%.1f}$\t$%.1f_{%.1f}$", FILE, REPORT, LOC_MEAN[ROWCOUNT]*100, LOC_STDDEV[ROWCOUNT]*100, LOC_MEAN[REPORT]*100, LOC_STDDEV[REPORT]*100)}
set grid
set yrange [0:1]
